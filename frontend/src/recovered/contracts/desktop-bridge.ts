...
export type DesktopUpdateState =
  | {
      type: "disabled";
      reason: "not-packaged" | "lab-build" | "unsupported-platform" | "disabled-by-env";
    }
  | {
      type: "idle";
      lastCheck?: { at: number; result: "up-to-date" | "error"; errorMessage?: string };
    }
  | { type: "checking" }
...
export interface McpDesktopBridge {
  ...
  remove(serverId: string): Promise<{
  ...
  toggleToolDisabled(args: {
    serverId: string;
    toolName: string;
  }): Promise<McpToolSummary[]>;
  onAuthCompleted(
    listener: BridgeListener<{ serverId: string; accountKey?: string; status: string }>,
  ): Unsubscribe;
}
...
export interface DesktopBridge {
  ...
  readonly attachProdBox: {
    ...
    setEnabled(
      enabled: boolean,
      options?: { isRestartMainApp?: boolean },
    ): Promise<unknown>;
  };
