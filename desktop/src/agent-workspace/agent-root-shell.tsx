...
import extra from './agent-root-shell.module.css';
import {
...
import {
  ...
  StoryViewer,
} from '../adapters/telegram/telegram-overlays';
import { CompatibilityCallDialog, CallsCompatibilityWorkspace } from '../adapters/calls/calls-compatibility-adapter';
import { PaymentInvoiceDialog, PaymentsCompatibilityWorkspace, type PaymentUiState } from '../adapters/payments/payments-compatibility-adapter';
import {
  MiniAppCompatibilityDialog,
  MiniAppsCompatibilityWorkspace,
  miniAppCloudBridgeDocument,
  type MiniAppMarketplaceProps,
} from '../adapters/miniapps/miniapps-compatibility-adapter';
import {
  SettingsCompatibilityModal,
  type SettingsWorkspaceProps,
} from '../adapters/settings/settings-compatibility-adapter';
...
async function saveInvoiceDialog() {
  ...
  return (
    ...
    >
      ...
      {invoiceDialog ? <PaymentInvoiceDialog dialog={invoiceDialog} onChange={setInvoiceDialog} onClose={() => setInvoiceDialog(null)} onSave={() => void saveInvoiceDialog()} /> : null}
      ...
      {section === 'settings' ? <SettingsCompatibilityModal
        category={settingsCategory}
        onCategory={setSettingsCategory}
        preferences={desktopPreferences}
        onPreference={updateDesktopPreference}
        actor={currentActor}
        actorId={selfHosted.actorId}
        hostSettings={hostSettings}
        onHostSetting={updateHostSetting}
        onConfigureProviderSecret={configureProviderSecret}
        onRemoveProviderSecret={removeProviderSecret}
...
function FeatureWorkspace({
  ...
  settings: SettingsWorkspaceProps;
} & MiniAppMarketplaceProps) {
  const fallback = section === 'contacts'
    ? <ContactsCompatibilityWorkspace />
    : section === 'chats'
      ? <TelegramCompatibilityWorkspace />
      : <div className={styles.featureWorkspace}><MessageCircle size={54} /><h2>{sectionTitle(section)}</h2><p>兼容消息域由独立 adapter 提供，AgentRootShell 只消费投影。</p></div>;
  return <CompatibilitySurface
    section={section}
    miniApps={<MiniAppsCompatibilityWorkspace {...miniAppProps} />}
    payments={<PaymentsCompatibilityWorkspace payment={payment} onInvoice={onInvoice} onRefund={onRefund} />}
    calls={<CallsCompatibilityWorkspace />}
