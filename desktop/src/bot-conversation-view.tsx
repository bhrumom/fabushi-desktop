          ...
          <div className={styles.transcript}>
            {messages.map((message, index) => {
              if (message.kind === 'assistant-turn' && message.assistantTurn) {
                return (
                  <MahayanaAssistantTurnView
                    ...
                    label={title}
                    avatar={<FabAvatar identity={botId} state={message.streaming ? 'working' : 'idle'} size={28} label={title} active={message.streaming} />}
                  />
                );
              }
              ...
              return (
                ...
                >
                  {!userMessage ? <FabAvatar identity={botId} state={message.streaming ? 'working' : 'idle'} size={28} label={title} active={message.streaming} /> : null}
