          ...
          <div className={styles.transcript}>
            {messages.map((message, index) => {
              if (message.kind === 'assistant-turn' && message.assistantTurn) {
                return (
                  <MahayanaAssistantTurnView
                    ...
                    label={title}
                    avatar={<FabAvatar identity={botId} state={message.streaming ? 'working' : 'idle'} active={Boolean(message.streaming)} size={28} label={title} />}
                  />
                );
              }
              ...
              if (message.kind === 'thinking') {
                return (
                  <div key={message.id} className={styles.thinkingRow} data-testid="agent-thinking" data-operation-id={message.operationId}>
                    <FabAvatar identity={botId} state="thinking" size={28} active label={title} />
              ...
              return (
                ...
                >
                  {!userMessage ? <FabAvatar identity={botId} state={message.streaming ? 'working' : 'idle'} active={Boolean(message.streaming)} size={28} label={title} /> : null}
