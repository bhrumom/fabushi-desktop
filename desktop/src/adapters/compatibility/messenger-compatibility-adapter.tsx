...
import {
  contactKindForLegacyConversation,
  contactKindForSelfHostedConversation,
} from '../contacts/contacts-compatibility-adapter';
import { telegramKindForLegacyConversation } from '../telegram/telegram-compatibility-adapter';
...
function legacyFallbackKind(conversation: ConversationSummary): CompatibilityPeerKind {
...
function selfHostedCompatibilityKind(
  conversation: MessagingConversation,
  counterparty?: MessagingActor,
): CompatibilityPeerKind {
  const directKind = contactKindForSelfHostedConversation(conversation, counterparty);
...
export function buildCompatibilityPeers(input: CompatibilityPeerProjectionInput): CompatibilityPeerItem[] {
  ...
  const legacyConversations = conversations.map((conversation): CompatibilityPeerItem => {
    ...
    const kind = bot
      ? 'bot'
      : telegramKindForLegacyConversation(conversation)
        ?? contactKindForLegacyConversation(conversation)
        ?? legacyFallbackKind(conversation);
    return {
  ...
  const nativePeers = selfConversations.map((conversation): CompatibilityPeerItem => {
    ...
    const kind = selfHostedCompatibilityKind(conversation, actor);
    return {
