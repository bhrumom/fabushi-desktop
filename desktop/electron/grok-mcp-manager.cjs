...
class HttpMcpClient{
  constructor(config,getAuthorizationHeader=async()=>null){this.config=config;this.getAuthorizationHeader=getAuthorizationHeader;this.nextId=1;this.initialized=false;this.sessionId=null;this.protocolVersion=PROTOCOL_VERSION}
  async headers(){const authorization=await this.getAuthorizationHeader(this.config);return{'content-type':'application/json',accept:'application/json, text/event-stream','mcp-protocol-version':this.protocolVersion,...(this.sessionId?{'mcp-session-id':this.sessionId}:{}),...(authorization?{authorization}:{})}}
...
function createMcpManager({getServers,getAuthorizationHeader=async()=>null}){const clients=new Map();
  const clientFor=server=>{let client=clients.get(server.id);const signature=JSON.stringify([server.transport,server.command,server.args,server.url,server.accountKey,server.oauthClientId,server.oauthAuthorizationUrl,server.oauthTokenUrl]);if(client&&client.signature!==signature){client.instance.dispose();clients.delete(server.id);client=null}if(!client){client={signature,instance:server.transport==='http'?new HttpMcpClient(server,getAuthorizationHeader):new StdioMcpClient(server)};clients.set(server.id,client)}return client.instance};
