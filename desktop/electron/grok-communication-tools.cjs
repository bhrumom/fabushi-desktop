...
const definitions=[
  {type:'function',function:{name:SEND_MESSAGE_TOOL_NAME,description:'Send something visible to the user in Fabushi. Use text for chat/progress, attachment for a file or media URL, widget for an interactive choice, and secret-request for a credential that must never be pasted into chat. The reference cursor-agent card is intentionally absent because Fabushi agents run on the installed computer rather than cloud agents.',parameters:{type:'object',properties:{
    ...
    content:{type:'string'},url:{type:'string'},alt:{type:'string'},reply_to:{type:'string'},channel:{type:'string'},
    images:{type:'array',items:{type:'object',properties:{url:{type:'string'},alt:{type:'string'}},required:['url'],additionalProperties:false}},
    widget:{type:'object',properties:{prompt:{type:'string'},helpText:{type:'string'},options:{type:'array',items:{type:'object',properties:{label:{type:'string'},value:{type:'string'},description:{type:'string'},style:{type:'string',enum:['default','primary','danger']}},required:['label'],additionalProperties:false}},allowCustom:{type:'boolean'},dismissOnMoveOn:{type:'boolean'}},required:['prompt','options'],additionalProperties:false},
...
function normalizeWidget(value){
  ...
  if(!Array.isArray(value.options)||value.options.length<1||value.options.length>6)throw Error('Widget requires 1 to 6 options.');
...
function normalizeSendMessage(args){
  ...
  const replyToId=String(args?.reply_to||'').trim()||null,channel=String(args?.channel||'').trim()||null;
  if(channel&&type!=='text'&&type!=='attachment')throw Error('channel is only valid with type:text or type:attachment.');
  const provided=value=>value!=null&&(typeof value!=='string'||value.length>0)&&(!Array.isArray(value)||value.length>0);
  const allowed={
  ...
  for(const [key,value] of Object.entries(args||{}))if(provided(value)&&!allowed.has(key))throw Error(key+' is not valid with type:'+type+'. Re-send it as a separate properly typed SendMessage call.');
  if(type==='text'){
    const content=nonEmpty(args?.content,'SendMessage content',100000);
    const images=Array.isArray(args?.images)?args.images.map(image=>({url:validAttachmentUrl(image?.url),...(String(image?.alt||'').trim()?{alt:String(image.alt).trim().slice(0,500)}:{})})):[];
    return{type,content,replyToId,channel,images};
  }
  if(type==='attachment')return{type,url:validAttachmentUrl(args?.url),alt:String(args?.alt||'').trim().slice(0,500),replyToId,channel};
