...
Target branch: `refactor/post-1.2.74-architecture-cutover-20260921` / PR #14
...
| Coordinator boundary | narrow lifecycle + command/event protocol, reconnection, pending requests outside view components | legacy renderer shell owned runtime, product and presentation concerns | IMPLEMENTED: Electron Host lifecycle is projected through the narrow bridge; Rust `RuntimeStore` journals turn inputs and startup recovery creates the next `ExecutionRun` generation for interrupted LogicalTurns. `AgentRuntimeCoordinator` only rebinds the recovered operation to the owning conversation/Agent and keeps drafts/history intact; `waiting-user` restart is fail-closed and requires explicit retry. |
...
| Transcript projection | ordered rich entries for text, reasoning, tools, approvals, attachments and timeline state | mixed DisplayMessage / AssistantTurn / legacy message paths | IMPLEMENTED: canonical `TranscriptEntry` + `AgentTranscriptStore` own Agent projection, request→operation adoption, AssistantTurn reduction, approval/tool/computer timeline entries and terminal settling. Regenerate/restart preserves one logical user message, removes the previous run projection and binds the next Rust run generation back to that same message; normal Agent rendering no longer round-trips through renderer-global `messages`. |
...
| Plugins / Computer | secondary overlays from Agent workspace | top-level messenger/product sections and profile panels | IMPLEMENTED: Plugins/Settings live behind Agent-owned surfaces; Computer is bound to `agentId` and the installed Fabushi machine. Renderer `useAgentComputerController` is a UI facade only: remote registration/session polling lives in Electron Main background services, privileged actions enter Rust `CapabilityBroker`, physical input is serialized by `ComputerControlLease`, and Agent-scoped status/snapshot/result/takeover lifecycle is pushed back to the owning Agent. |
...
| Legacy messenger | not the primary desktop product shell | dominates navigation and renderer architecture | COMPATIBILITY-ONLY: no hidden legacy main navigation remains and `AgentRootShell` is the sole root. Deleted `messaging-shell-v2.tsx` / `legacy-messaging-shell.tsx` do not exist; Contacts/Telegram/Mini App and other secondary compatibility routing is isolated behind explicit adapters that do not own Agent runtime state. |

## 2026-09-20 Agent workspace extraction

The initial extraction started on PR #7; the current architecture-completion continuation is PR #14 (`refactor/post-1.2.74-architecture-cutover-20260921`).
...
P0: CODE COMPLETE for the source architecture. The remaining gate is fresh exact-HEAD GitHub Actions plus packaged macOS acceptance; no further product-shell migration is expected before that gate unless CI exposes a defect.

P1: DONE for shell ownership: AgentRootShell, AgentWorkspaceController, AgentTranscriptStore, AgentRuntimeCoordinator, Agent directory/navigation projection, Sidebar/Network/Command Palette/Workflow/Store-Sync controllers, AgentHeader, Transcript, Composer and overlays are extracted. Contacts/Telegram/Mini App peer construction, messaging-envelope recognition and secondary-surface routing sit behind explicit compatibility adapters; both legacy messaging shell files are deleted.
...
P3: DONE for the primary Agent path: HostProcess lifecycle is replayed through the Electron edge, while Rust `RuntimeStore` owns LogicalTurn/ExecutionRun restart recovery. Interrupted non-terminal work resumes as generation N+1 from durable turn input; the renderer replaces the previous run projection and reclaims the new operation by conversation identity. `waiting-user` is never silently resumed after restart: its old run is failed and explicit user retry is required.
...
- Desktop version remains `1.2.74` for the architecture PR. After the exact-HEAD architecture gate passes, PR #14 must merge without source mutation; only then may canonical `main` receive the monotonic release bump (at least `1.2.75`). Signed/notarized publication must use the exact merged release-version SHA.

The remaining gate is verification, integration and release: use the existing PR #14 on `refactor/post-1.2.74-architecture-cutover-20260921`, require Desktop Chat Parity, Rust desktop runtime and packaged acceptance on one exact immutable HEAD, merge only when all are green, then create the monotonic release-version change from canonical `main` and publish macOS only from that exact accepted release SHA.
