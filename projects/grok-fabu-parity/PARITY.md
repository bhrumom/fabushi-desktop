...
P1: continue shrinking the monolithic compatibility renderer now that AgentRootShell, AgentWorkspaceController, AgentTranscriptStore, AgentRuntimeCoordinator, the workspace runtime facade, Agent navigation projection, Sidebar controller, AgentSidebar, AgentHeader, Transcript, Composer, Network, Command Palette and overlays are extracted. Messenger no longer owns the primary Agent runtime, navigation projection or Sidebar state lifecycle; the remaining work is to move compatibility-only peer construction/rendering/event branches behind dedicated adapters, then delete those branches from the primary renderer.
