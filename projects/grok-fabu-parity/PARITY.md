...
| Coordinator boundary | narrow lifecycle + command/event protocol, reconnection, pending requests outside view components | `messaging-shell-v2.tsx` owns too many runtime, product and presentation concerns | IN PROGRESS: `AgentWorkspaceController` owns Agent-scoped request/draft/upload state, `AgentCoordinatorClient` owns commands, and `AgentRuntimeCoordinator` now exclusively owns normal Agent event attribution, request→operation adoption, delta backpressure and terminal settlement; transport-level reconnect/handshake extraction remains |
...
P3: move the remaining transport-level Host lifecycle behind the coordinator boundary: explicit reconnect/lifecycle handshake, transport health/recovery and non-Agent event families. Normal Agent pending request adoption, streaming and terminal event reduction are no longer renderer-owned.
