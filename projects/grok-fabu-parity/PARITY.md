...
| Cloud Agent store | immutable CAS blobs + mutable root/reference state | renderer cloud sync was profile-oriented | IN PROGRESS: profile/memory/workflow/automation/transcript mirror through account Agent Store; `root.json` is now validated and restored before cross-device transcript hydration, while full attachment/runtime-checkpoint CAS restoration remains |
...
| Plugins / Computer | secondary overlays from Agent workspace | top-level messenger/product sections and profile panels | IN PROGRESS: Plugins/Settings moved to Agent footer; Computer is a first-class Agent overlay explicitly bound to `agentId` and the installed Fabushi machine; Agent-scoped snapshot/action results now project into the owning Agent timeline as `computer-handoff`; OS Accessibility/Screen Recording permission state remains a Computer capability surface |
...
- `desktop/src/agent-workspace/coordinator-client.ts`: narrow Mahayana command boundary for send/open/broadcast/interrupt/approval and `attachment.upload -> attachment.stored`; it now owns retry/backoff and generation-aware recovery handshake instead of delegating reconnect to React.
...
- `desktop/scripts/check-agent-workspace-boundary.mjs`: CI-enforced architecture guard that rejects reintroduction of renderer-global Agent operation pointers, queued Agent transcript maps, Agent delta buffers, Agent transcript copy-back through `messages`, or direct Host subscription ownership.

This is not a claim of full parity. Operation-scoped approval request/resolution and generation-aware Host recovery are now owned by the Agent runtime boundary, and `root.json` can restore a cross-device conversation snapshot without an empty local Host overwriting it. A full rich-text/provider editor, remaining OS-level Computer permission/status projection, full attachment/runtime-checkpoint CAS restoration, host-backed sidebar section sync and extraction of the remaining compatibility Messenger renderer branches remain open gates.
...
P2: finish the remaining richer renderer parity: full rich-text/provider mention editor, Agent settings/profile editor, OS-level Computer permission/status presentation, and host-backed cross-device section state. Approval and Agent-scoped Computer handoff timeline projection, draft/waiting/activity, attachment timeline, pinned drag, custom sections and multi-select are implemented.
...
P4: IN PROGRESS: cross-device `root.json` validation and transcript restoration are implemented with etag/conflict-safe writes; complete attachment index/runtime checkpoint/CAS graph materialization is still required.
