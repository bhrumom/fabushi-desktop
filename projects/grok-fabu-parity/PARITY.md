...
| Agent Composer | Agent-scoped draft/reply/attachments/drop/paste/voice; transport acceptance independent from Agent turn state | textarea plus Messenger attachment/reply behavior | IN PROGRESS: TipTap 3.14.0, multi-file/drop/paste, attachment-only sends, restart-safe Agent draft recovery, semantic reply context and voice dictation are implemented. Voice transcription inserts at the active editor selection, IME/Escape/send gating follows the frozen Fabu keyboard contract, and all file entry paths enforce the six-file Agent limit before upload. `@Agent`, enabled workflow and normalized `@MCP` selections persist stable ids through queued-send failure/recovery. Link marks now preserve their target in Mahayana's normalized text projection. Emoji/PR-reference and the remaining Fabu suggestion-provider surfaces still remain. |
...
| Groups / Agent network | first-class Agent group/org semantics | group/channel semantics are primarily messenger/community based | IN PROGRESS: the primary Network surface is Agent-owned and backed by Mahayana `group.list/create/update/delete/send`, direct `agent.send/agent.peerHistory` handoff, and `agent.broadcast` through `AgentCoordinatorClient`. `useAgentNetworkController` now owns its own group projection and peer history instead of reading Messenger's `groups` state; legacy groups receive the same Host group events only as a compatibility projection. Full org-graph visualization and richer multi-stage coordination UX remain. |
...
This is not a claim of full parity. Operation-scoped approval request/resolution and generation-aware Host recovery are owned by the Agent runtime boundary. `root.json` now materializes and validates its full CAS graph before transcript/attachment/checkpoint hydration; stale running checkpoints fail closed into an interrupted recovery notice. Computer permission/status projection and Native Host sidebar durability are implemented. Remaining gates are emoji/PR-reference and per-Agent provider parts of the Fabu editor, account-level cross-device sidebar sync, extraction of the remaining compatibility Messenger renderer branches, richer org-graph coordination UI, and packaged macOS parity acceptance. Fabu profile fields `name/title/description/avatarShape/avatarColor` plus notifications are now carried through the Agent settings controller; hidden-from-sidebar remains correctly owned by Sidebar state.
